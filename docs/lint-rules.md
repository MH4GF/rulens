...
Rules that ensure effective testing practices when using Vitest.
...
| [`no-restricted-vi-methods`](https://github.com/vitest-dev/eslint-plugin-vitest/blob/main/docs/rules/no-restricted-vi-methods.md) | disallow specific `vi.` methods (error, with options) |
